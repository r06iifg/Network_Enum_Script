...
class NmapScanner:
    ...
    def run_tcp_scan(self):
        nmap_tcp_command = f"nmap -sS -p- -sV -sC -O {self.ip}"
        nmap_tcp_output = run_command(nmap_tcp_command)
        print(f"TCP Scan Output:\n{nmap_tcp_output}")  # Debugging output
        self.output += f"Below is TCP scan output for {self.ip}:\n" + nmap_tcp_output + "\n"

    def run_udp_scan(self):
        nmap_udp_command = f"nmap -sU -p- -sV -sC -O {self.ip}"
        nmap_udp_output = run_command(nmap_udp_command)
        print(f"UDP Scan Output:\n{nmap_udp_output}")  # Debugging output
        self.output += f"Below is UDP scan output for {self.ip}:\n" + nmap_udp_output + "\n"
    ...
    def run_port_specific_scans(self, port):
        if port == '21':
            ...
            self.output += run_command(f'python3 sshaudit.py {self.ip}') + "\n"
            self.output += run_command(f'python3 Libauth.py {self.ip}') + "\n"
            ...
            self.output += run_command(f'python3 enum4linux-ng {self.ip}') + "\n"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Nmap scans on specified IP addresses.")
    parser.add_argument('--udp', action='store_true', help="Include UDP scans.")
    ...
    for ip in ips:
        ...
        if ip:
            scanner = Scanner(ip)
            scanner.run(args.udp)  # Pass the --udp argument
